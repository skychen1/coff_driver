...
class CRC {
    ...
    public static byte[] getCRC(byte[] byteArr, int length) {
        byte uchCRCHi = (byte) 0xFF;
        byte uchCRCLo = (byte) 0xFF;
        int uIndex = 0;
        byte[] bArr = {0, 0, 0, 0};
        byte[] bArr1 = {0x00, 0x00};
        for (int i = 0; i < length; i++) {

            bArr[3] = (byte) (uchCRCHi ^ byteArr[i]);
            uIndex = DataSwitcher.bytesToInt(bArr);
    ...
    public static byte[] getCRC(byte[] byteArr, int from, int to) {

        byte[] mbyteArr = new byte[to - from + 1];
        for (int i = from; i <= to; i++) {

            mbyteArr[i - from] = byteArr[i];
        }
        return getCRC(mbyteArr, mbyteArr.length);
    }

    public static byte[] getTempSensorCRC(int No) {
